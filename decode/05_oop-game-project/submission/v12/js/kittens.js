...
// Preload game images
var images = {};
['moveLeft1.png','moveLeft2.png','moveLeft3.png','moveRight1.png','moveRight2.png','moveRight3.png','bg.jpg', 'heart.png','cloud1.png','cloud2.png', 'donut.png', 'donut2.png', 'player.png', 'player2.png', 'salad.png', 'salad2.png', 'playerleft.png'].forEach(imgName => {
...
class Player {
    constructor() {
        ...
        this.spriteLeft3 = images['moveLeft3.png']
        this.spriteRight3 = images['moveRight3.png']
    ...
    render(ctx, randomMove) {
        if(this.movingLeft === true || this.movingRight === true){
            if(this.movingLeft === true){
                if(this.walkingAnimationDelay%2 === 0){
                    this.imgSelector = 'spriteLeft'+(this.walkingAnimationFrame%3+1) 
                ...
                if(this.walkingAnimationDelay%2 === 0){
                    this.imgSelector = 'spriteRight'+(this.walkingAnimationFrame%3+1)
                    this.walkingAnimationFrame++
                }
                this.walkingAnimationDelay++
                ctx.drawImage(this[this.imgSelector], this.x + randomMove, this.y);
...
class Engine {
    ...
    start() {
        ...
        document.addEventListener('keydown', e => {
            if (e.keyCode === LEFT_ARROW_CODE) {
                //this.player[0] = this.playerleft;
                //this.playerleft.move(MOVE_LEFT);
                this.player[0].move(MOVE_LEFT);   
                footsteps.play();
            }
            else if (e.keyCode === RIGHT_ARROW_CODE) {
                this.player[0].move(MOVE_RIGHT);
                footsteps.play()
            }
        ...
        document.addEventListener('keyup', e => {
            if (e.keyCode === LEFT_ARROW_CODE) {
                this.player[0].stopMovingSideway()
                footsteps.pause();
            }
            else if (e.keyCode === RIGHT_ARROW_CODE) {
                this.player[0].stopMovingSideway()
    ...
    gameLoop() {
       ...
       //this.player.render(this.ctx, this.randomMove)
        ...
        this.friends.forEach((friend, friendIdx) => {
            if (friend.y > GAME_HEIGHT - (GAME_HEIGHT - this.player[0].y)) {
    ...
    isPlayerDead() {
        ...
        return this.enemies.some(function (enemy) {
            return (enemy.x === self.player[0].x && enemy.y + ENEMY_HEIGHT >= self.player[0].y + 20)
                && (enemy.y < self.player[0].y) + ENEMY_HEIGHT - 100;
    ...
    friendBonus() {
        ...
        return this.friends.some(function (friend) {
            return (friend.x === self.player[0].x && friend.y + FRIEND_HEIGHT >= self.player[0].y + 20)
                && (friend.y < self.player[0].y) + FRIEND_HEIGHT - 100;
        })
