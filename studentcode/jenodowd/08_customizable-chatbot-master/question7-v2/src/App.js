...
class App extends Component {
  ...
  settingsFinished = (name) => {
    var allMsgs = this.state.allMsgs
    allMsgs.forEach((element,index)=>{
      if(element.userName === this.state.username){
        allMsgs[index].userName = name 
      }
    })
    this.setState({ username: name, screen: CHAT_SCREEN, allMsgs : allMsgs });
  }
  gotoSettings = () => {
  ...
  deleteItem = (index) => {
    console.log(index)
    var tempArray = this.state.allMsgs
    tempArray = tempArray.filter((el,id)=>{
      return (
        id !== index
      )
    })
